...
class TritonRMSNorm2d(nn.LayerNorm):
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Fallback to pure PyTorch implementation to avoid Triton compilation (missing gcc)
        # RMSNorm: x * (1 / sqrt(mean(x**2) + eps)) * gamma + beta
        # Assumes x is (B, C, H, W) and we norm over C (dim 1)
        
        mean_square = torch.mean(torch.square(x), dim=1, keepdim=True)
        rrms = torch.rsqrt(mean_square + self.eps)
        out = x * rrms
